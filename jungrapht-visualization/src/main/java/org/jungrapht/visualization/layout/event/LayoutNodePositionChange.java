...
public interface LayoutNodePositionChange {
  ...
  class NetworkEvent<N> extends Event<N> {
    final Graph<N, ?> network;

    public NetworkEvent(N node, Graph<N, ?> network, Point location) {
      super(node, location);
      this.network = network;
    }

    public NetworkEvent(Event<N> layoutEvent, Graph<N, ?> network) {
      super(layoutEvent.node, layoutEvent.location);
      this.network = network;
    }

    public Graph<N, ?> getNetwork() {
      return this.network;
    }
  }
