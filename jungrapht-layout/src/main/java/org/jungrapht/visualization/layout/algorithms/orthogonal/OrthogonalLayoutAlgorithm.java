...
//        }
    ...
    Map<V, Rectangle> vertexToRectangleMap = new HashMap<>();
    Map<Rectangle, V> rectangleToVertexMap = new HashMap<>();
    Function<V, Rectangle> identityVertexDimensionFunction =
            v -> Rectangle.IDENTITY;
    Function<V, Rectangle> realVertexDimensionFunction;
    Function<V, Rectangle> vertexDimensionFunction;

    @Override
    public void visit(LayoutModel<V> layoutModel) {
        this.vertexToRectangleMap = new HashMap<>();
...
//        k=(0.2/T)1/iterationCount ;
        ...
        for (; iteration < iterationCount / 2; iteration++) {
            for (V v : graph.vertexSet()) {
                Point neighborsMedian = neighborsMedianPoint(v)
                ...
                if (occupiedRectangles.contains(rectangle)) {
                    ...
                    if (winner == rectangle) {
                        ...
                        V closestCellVertex = rectangleToVertexMap.get(closest);
                        updateMaps(closestCellVertex, rectangle);
                        updateMaps(v, closest);
...
//            log.info("cells size: {}", vertexToRectangleMap.size());
        ...
        for (int i=iterationCount / 2 + 1; i < iterationCount; i++) {
            ...
            for (V v : graph.vertexSet()) {
                ...
                Point neighborsMedian = neighborsMedianPoint(v)
                        .add(randomTemp(-temperature*vd.width, temperature*vd.width),
...
//            log.info("cells size: {}", vertexToRectangleMap.size());
            if (iterationCount % 9 == 0) {
                compact(compactionDirection,
                        Math.max(1, 1 + 2*(iterationCount - i - 30)/0.5*iterationCount), false);
                compactionDirection = !compactionDirection;
            }
            temperature = temperature * k;
        }
...
//        centerIt(layoutModel, Collections.emptyList());
    ...
    private void updateMaps(V v, Rectangle r) {
    ...
    private void compact(boolean direction, double gamma, boolean expand) {
        ...
        Compaction.of(cells, direction, gamma);
    }
...
//        Map<V, Rectangle> cells = new HashSet<>();
        ...
        graph.vertexSet().forEach(v -> {
            ...
            Rectangle rectangle = Rectangle.of(x, y, 1, 1);
            // no dupes
            while (rectangleToVertexMap.keySet().contains(rectangle)) {
...
//        return cells;
    ...
    Collection<Rectangle> findNearbyEmptyCells(Rectangle cell, int dist, Set<Rectangle> occupiedRectangles) {
        // compute once
        ...
        List<Rectangle> neighbors = neighborsOf(cell, dist)
                .stream()
                .filter(c -> !occupied.contains(c)).collect(Collectors.toList());
    ...
    Set<Rectangle> occupiedRectangles() {
        return vertexToRectangleMap.values().stream().collect(Collectors.toSet());
    }
...
//                                    -vertexDimensionFunction.apply(v).height / 2);
    ...
    Function<V, Point> locationModel = v -> {
        Rectangle cell =
                vertexToRectangleMap.get(v);
...
//        return Point.ORIGIN;
    ...
    int distance(Rectangle cell,Rectangle neighborCell) {
        return (int)(Math.abs(cell.x - neighborCell.x) +
                Math.abs(cell.y - neighborCell.y));
    }

    double sumOfDistToNeighbors(V v) {
        return Graphs.neighborSetOf(graph, v).stream()
                .flatMapToDouble(n -> DoubleStream.of(distance(v, n))).sum();
    }
    double sumOfDistToNeighbors(V v, Rectangle cell) {
        return Graphs.neighborSetOf(graph, v).stream()
                .flatMapToDouble(n -> DoubleStream.of(distance(cell, vertexToRectangleMap.get(n)))).sum();
    }
        ...
        List<Point> points =
                graph.degreeOf(v) > 0 ?
                        ...
                        Graphs.neighborSetOf(graph, v)
                                .stream()
                                .map(w -> vertexToRectangleMap.get(w))
    ...
    List<Rectangle> sameY(double y, List<Rectangle> in) {
        List<Rectangle> sorted = in.stream().filter(c -> c.y == y)
                .collect(Collectors.toList());
    ...
    List<Rectangle> sameX(double x, List<Rectangle> in) {
        List<Rectangle> sorted = in.stream().filter(c -> c.x == x)
                .collect(Collectors.toList());
