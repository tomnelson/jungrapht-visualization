...
/**
 * a double mapping of vertex to Point and Point to vertex. As each Point can locate only one
 * vertex, any update of a vertex to a new Point must first remove the previous rectangleToVertex
 * mapping
 *
 * @param <V>
 */
public class Mappings<V> {
  ...
  void confirmIntegrity() {
    if (vertexToPointMap.size() != pointToVertexMap.size()) {
      throw new IllegalArgumentException(
          "sizes differ " + vertexToPointMap.size() + " != " + pointToVertexMap.size());
    }
    for (V v : vertexToPointMap.keySet()) {
      Point r = vertexToPointMap.get(v);
      if (!pointToVertexMap.get(r).equals(v)) {
        throw new IllegalArgumentException(
            "Mismatch in Mappings " + vertexToPointMap + " and " + pointToVertexMap);
      }
    }
  }
  ...
  private boolean update(V v, Point r) {
    if (r == null) {
      throw new IllegalArgumentException("Point cannot ne null");
    }
    if (v == null) {
  ...
  protected void normalize(int min) {
    Collection<Point> mins =
        rectangles()
            .stream() //.map(Rectangle::min)
            .collect(Collectors.toSet());
    Rectangle extent = computeExtent(mins);
    double offsetX = -extent.x;
    //extent.x < 0 ? -extent.x : min;
    double offsetY = -extent.y;
    //extent.y < 0 ? -extent.y : min;
    Map<V, Point> newMap = new HashMap<>();
    entries().forEach(e -> newMap.put(e.getKey(), e.getValue().add(offsetX, offsetY)));
    forceUpdates(newMap.entrySet());
  }

  protected void expand(int size) {
    this.normalize();
    Rectangle extent = this.computeExtent();
    double xExpand = size / extent.width;
    double yExpand = size / extent.height;
    ...
    this.entries()
        .forEach(
            e ->
                newMap.put(
                    e.getKey(),
                    Point.of((int) (e.getValue().x * xExpand), (int) (e.getValue().y * yExpand))));
    //    this.entries().forEach(e -> newMap.put(e.getKey(),
    //            Point.of(e.getValue().x * xExpand, e.getValue().y * yExpand)));
  ...
  public String toString() {
    return "Mappings{" + "vertexToRectangleMap=" + vertexToPointMap + '}';
  }
}
