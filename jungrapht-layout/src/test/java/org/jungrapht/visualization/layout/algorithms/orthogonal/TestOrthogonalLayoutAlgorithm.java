...
public class TestOrthogonalLayoutAlgorithm {
  ...
  public void before() {
    graph =
        GraphTypeBuilder.<String, Integer>forGraphType(DefaultGraphType.dag())
            ...
            .buildGraph();
    layoutModel = LayoutModel.<String>builder().graph(graph).size(100, 100).build();
    mappings = new Mappings<>();
    layoutAlgorithm = new OrthogonalLayoutAlgorithm(layoutModel);
  }
    ...
    IntStream.range(0, 10)
        .forEach(
            n -> {
              graph.addVertex("V" + n);
              mappings.accept(
                  "V" + n, Point.of(Math.random() * 100 - 50, Math.random() * 100 - 50));
            });
  ...
  @Test
  public void testShowNeighbors() {
    List<Point> list = layoutAlgorithm.neighborsOf(Point.of(5, 5), 1);
    log.info("list: {}", list);
    list = layoutAlgorithm.neighborsOf(Point.of(5, 5), 2);
    log.info("list: {}", list);
    list = layoutAlgorithm.neighborsOf(Point.of(5, 5), 3);
    log.info("list: {}", list);

    list = layoutAlgorithm.neighborsOf(Point.of(5, 5), 1.0);
    log.info("list: {}", list);
    list = layoutAlgorithm.neighborsOf(Point.of(5, 5), 2.5);
    log.info("list: {}", list);
    list = layoutAlgorithm.neighborsOf(Point.of(5, 5), 3.5);
    log.info("list: {}", list);
  }
